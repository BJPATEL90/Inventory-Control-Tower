...
async function apiCall(action, params = {}, opts = {}) {
  ...
  if (showLoader && typeof showGlobalLoader === 'function') showGlobalLoader(true);
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (showLoader && typeof showGlobalLoader === 'function') showGlobalLoader(false);
      return json.data;
  ...
  if (typeof showToast === 'function') showToast(`Failed to load ${action}. ${lastError.message}`, 'error');
  throw lastError;
}
